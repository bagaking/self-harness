...
3. Apply the feedback-pressure ratchet when the task includes supervisor feedback, low-value loop feedback, or a request to raise the bar.
   ...
   - When reviewing whether a newly added supervisor commit-gate check actually ran, distinguish same-run stable-copy commits from next-run checked-out supervisor activation. If `.self-harness/tmp/commit-gate-last-report.md` only shows shell syntax validation for a newly wired check after `scripts/supervisor.sh` changed in that same run, cite `scripts/supervisor-stable-copy-check.sh` as the handoff proof and require the next checked-out supervisor commit report to emit the new check's own `ok` line.
   ...
   - When feedback says the loop still stops too easily or asks when the branch may stop, run `scripts/branch-stop-condition-check.sh --run-limit 5 --trigger-limit 8 --evidence-limit 3`. Treat a failure as unresolved proof debt. Treat a pass as stop-safe only for the sampled branch-local pressure line, not as return-to-main readiness. Completed `Next supervisor pressure:` debt needs an explicit source marker such as `next-pressure-source: <source-outbox>` or an accepted pressure-specific marker like `continuous-pressure-source: <source-outbox>`; do not treat an arbitrary mailbox lifecycle path mention as proof. Explicit human/supervisor feedback that produced a feedback-pressure run remains a continuous-pressure source when the run uses `No next supervisor pressure:` until a later mailbox lifecycle record names that outbox with `continuous-pressure-source: <source-outbox>`; prove this with `scripts/continuous-supervisor-pressure-check.sh`. If the feedback targets the supervisor idle skip path, also prove `scripts/idle-stop-proof-fixture-check.sh` so a failed stop check seeds a defect-specific inbox with a bounded sanitized failure excerpt instead of becoming a silent idle stop or a private `.self-harness/tmp/` pointer.
